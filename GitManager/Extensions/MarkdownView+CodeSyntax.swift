//
//  MarkDownView+CodeSyntax.swift
//  GitManager
//
//  Created by Антон Текутов on 11.05.2020.
...
extension MarkdownView {
    ...
    @objc public func loadWithCodeSyntax(content: String, fileExtension: String) {
        
        updateSyntax()
        if fileExtension != "md" && fileExtension != ".md" {
            if let language = MarkdownView.syntax![fileExtension] ?? MarkdownView.syntax![".\(fileExtension)"] {
                let content = "\n\n ```\(language)\n\(content)```"
    ...
    private func updateSyntax() {
        guard MarkdownView.syntax == nil 
            else { return }
        MarkdownView.syntax = Dictionary<String, String>()
        let guides = MarkdownView.guides.split(separator: "\n")
        var currentTitle = ""
        for line in guides {
            if !line.isEmpty {
                ...
                if line.first == "." {
                    let fileExtension = String(line.dropLast())
                    MarkdownView.syntax![fileExtension] = currentTitle
                }
            }
        }
    }
    
    private static var syntax : Dictionary<String, String>?

    private static let guides = """
1C Enterprise:
  type: programming
  color: "#814CCC"
  extensions:
